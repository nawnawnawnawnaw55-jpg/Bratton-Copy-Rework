  ...
  const REN = {
    async loadPage(file, tabId) {
      ...
      try {
        ...
        // Homepage uses a full structural HTML template instead of snippet-based
        // reconstruction — the snippet renderer loses the <main.homepage> wrapper,
        // dividers, inline scripts, and section scaffolding that the live site needs.
        const isHomepage = (file === "index.html" || file === "/index.html");
        if (isHomepage) {
          await this._renderHomepageFromTemplate(tab);
        } else {
    ...
    async _checkJenaEditedPage(file, tab) {
      try {
        ...
        STATE.pristineSnapshots = {};
        for (const snippet of tab.snippets) {
          const sel = snippet._location?.selector;
          if (!sel) continue;
          try {
    ...
    _toggleJenaEdits() {
      ...
      if (STATE.jenaActive) {
        ...
        DOM.btnJenaToggle.innerHTML = "✏️ Show Edited";
        DOM.currentPageLabel.textContent = tab.label;
        this.renderSnippets(tab);
...
if (!src || src.startsWith("data:") || src.startsWith("files/") || src.startsWith("/files/") || src.startsWith("/assets/") || src.startsWith("assets/")) return;
    ...
    /**
     * Render the homepage using the full structural HTML template from
     * _vercel_homepage.html instead of stitching snippets together.  The
     * snippet-based reconstruction loses the <main.homepage> wrapper,
     * section scaffolding, inline scripts, dividers, and script tags that
     * the live site depends on for carousels, phase switching, reviews,
     * scroll reveals, and responsive layout.
     *
     * This method loads _vercel_homepage.html as structural scaffolding
     * and then overlays each snippet's editable content onto the matching
     * DOM region, preserving all the live-site scripts and structure.
     */
    async _renderHomepageFromTemplate(tab) {
      try {
        // 1. Fetch the live-site homepage template
        const resp = await fetch("_vercel_homepage.html");
        if (!resp.ok) throw new Error("Failed to load homepage template: " + resp.status);
        ...
        // 3. Extract the <main> element and its contents
        const templateMain = templateDoc.querySelector("main.homepage");
        if (!templateMain) throw new Error("Homepage template missing <main.homepage>");

        // 4. Create a fresh container for the preview
        const container = document.createElement("div");
        container.className = "br-page-preview homepage-preview";

        // 5. Clone the template main into the container (deep clone preserves scripts)
        const mainClone = templateMain.cloneNode(true);
        container.appendChild(mainClone);

        // 6. Build a lookup of snippets by their selector/position
        const snippets = tab.snippets || [];
        // Sort snippets by selector for positional matching
        const sortedBySelector = [...snippets].sort((a, b) =>
          (a.selector || "").localeCompare(b.selector || "")
        );

        // 7. Overlay snippet content onto the template DOM sections.
        //    We map snippets to DOM elements using CSS selectors derived
        //    from the snippet's selector property (e.g. "section.hero",
        //    "section.services", etc.)
        for (const snippet of snippets) {
          const sel = (snippet.selector || "").trim();
          if (!sel) continue;

          // Try to find the matching element in the cloned template
          let targetEl = null;
          try {
          ...
          //   - _newHtml (user-edited, not yet saved to GitHub)
          //   - liveHtml (fetched from live site)
          //   - originalHtml (fallback)
          const htmlToRender = snippet._newHtml || snippet.liveHtml || snippet.originalHtml;
          if (!htmlToRender) continue;

          // Parse the snippet HTML to extract its meaningful content
          const snippetDoc = parser.parseFromString(htmlToRender, "text/html");
          const bodyContent = snippetDoc.body;

          // Wrap the target element in a .br-snippet wrapper for editing
          const wrapper = document.createElement("div");
          wrapper.className = "br-snippet";
          wrapper.dataset.snippetId = snippet.id;
          wrapper.dataset.selector = sel;

          // Replace targetEl with the snippet's body content inside the wrapper
          targetEl.parentNode.insertBefore(wrapper, targetEl);

          // Use the snippet's HTML if it has meaningful content; otherwise keep template
          const hasContent = bodyContent.children.length > 0 ||
          ...
          // Mark the target element reference for cleanup
          targetEl._replaced = true;
        }

        // 8. Remove any template elements that were NOT replaced (clean remnants)
        // These are structural elements like <main>, <script>, etc. that don't
        // need editing — we keep them but don't wrap them in .br-snippet.
        // Actually, just remove orphan elements that were marked replaced.
        container.querySelectorAll("*").forEach(el => {
          if (el._replaced) {
            // Element was already moved into a wrapper — remove from old spot
            // (it should already be detached, but clean up just in case)
          }
        });

        // 9. Set up editable regions on all .br-snippet wrappers
        const allWrappers = container.querySelectorAll(".br-snippet");
        allWrappers.forEach(wrapper => {
          ...
          wrapper.addEventListener("input", () => {
            ...
            const newHtml = wrapper.innerHTML;
            TABS.markDirty(tab.id);
            // Update the snippet in-memory
            const snip = (tab.snippets || []).find(s => s.id === snippetId);
            if (snip) snip._newHtml = newHtml;
          });
        });

        // 10. Render into the page
        DOM.pageRender.innerHTML = "";
        DOM.pageRender.appendChild(container);

        // 11. Execute scripts in the cloned template (they run automatically
        //     when appended to the live DOM since we used cloneNode(true))
        //     — but re-run inline scripts that may have been stripped.
        ...
        console.error("Homepage template render failed, falling back to snippets:", err);
        // Fall back to regular snippet-based rendering
        this.renderSnippets(tab);
      }
    },
  ...
  const SUBS = {
    ...
    async submit() {
      ...
      toast("Sending changes to Discord and GitHub...", "info", 8000);
      ...
      try {
        ...
        toast("✓ Sent to Discord with file attachments!", "success", 6000);

        // Save via GitHub API
        if (token) {
          await SUBS.saveToGitHub(dirty, token, repo);
        } else {
          toast("No GitHub token set — changes were NOT saved to GitHub.", "warning", 6000);
          githubOk = false;
        }
        ...
        toast("Submit failed — your changes are preserved. " + e.message, "error", 10000);
      }
    },
      ...
      for (const [rawFile, snaps] of Object.entries(filesMap)) {
        try {
          ...
          if (!fileContent || fileContent.trim().length === 0) {
            ...
            toast(`⚠ Could not load base content for ${rawFile} — edit NOT saved to GitHub. Check browser console.`, "error", 8000);
          ...
          for (let attempt = 0; attempt < MAX_RETRIES && !saved; attempt++) {
            ...
            if (putRes.ok) {
              ...
              toast("✓ Saved to GitHub", "success", 8000);
              saved = true;
            } else {
