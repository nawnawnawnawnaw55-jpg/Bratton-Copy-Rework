/**
 * Generate the two missing library parent pages (v3) from v2 sources.
 * - library_health → /library/library_health/
 * - library_nl_all → /library/library_nl_all/
 */
const fs = require('fs');
const path = require('path');

const V2 = 'bratton-pt-v2';
...
function makeBodyDiagram() {
  ...
  <div class="ml-svg-wrap">
    <svg id="med-lib-body" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 125.75 337.72" preserveAspectRatio="xMinYMin meet">
...
function makeCallButton() {
  return `<p style="margin-top:32px;text-align:center"><a href="tel:9856415825" class="btn btn--accent btn--lg">Questions? Call Us</a></p>`;
}
...
function cleanParentArticleList(rawHtml) {
  if (!rawHtml) return '<p style="text-align:center;color:#888">No articles available for this topic. Please browse our <a href="/library/">Medical Library</a>.</p>';
...
function generateHealth() {
  const v2html = fs.readFileSync(path.join(V2, 'library_health', 'index.html'), 'utf8');

  // Extract title
  const tm = v2html.match(/<title>(.*?)<\/title>/i);
  const title = tm ? tm[1].replace(/\s*\|\s*Bratton Physical Therapy.*/i, '').trim() : 'Medical Library Health';

  // Extract meta description
  const dm = v2html.match(/<meta\s+name=['"]description['"]\s+content=['"](.*?)['"]/i);
  const metaDesc = dm ? dm[1].trim() : 'Medical Library Health resources from Bratton Physical Therapy in Slidell, LA.';

  // Extract ml-article content (article list)
  const am = v2html.match(/<div\s+id=['"]ml-article['"][^>]*>([\s\S]*?)<\/div>/i);
  let articleList = '';
  if (am) {
    const raw = am[1].trim();
    // If it says "No Articles Available", use fallback
    if (raw.includes('No Articles Available')) {
      articleList = '<p style="text-align:center;color:#888">No articles available for this topic. Please browse our <a href="/library/">Medical Library</a>.</p>';
    } else {
      articleList = cleanParentArticleList(raw);
    }
  } else {
    articleList = '<p style="text-align:center;color:#888">No articles available. Please browse our <a href="/library/">Medical Library</a>.</p>';
  }

  const bodyContent = `<h1>Medical Library Health</h1>
...
${makeCallButton()}`;
  ...
  const outDir = path.join(LIB, 'library_health');
  if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
  const page = buildPage(title, metaDesc, bodyContent);
  fs.writeFileSync(path.join(outDir, 'index.html'), page, 'utf8');
  console.log('Generated: library/library_health/index.html');
}

function generateNlAll() {
  const v2html = fs.readFileSync(path.join(V2, 'library_nl_all', 'index.html'), 'utf8');

  // Extract title
  const tm = v2html.match(/<title>(.*?)<\/title>/i);
  const title = tm ? tm[1].replace(/\s*\|\s*Bratton Physical Therapy.*/i, '').trim() : 'Medical Library';

  // Extract meta description
  const dm = v2html.match(/<meta\s+name=['"]description['"]\s+content=['"](.*?)['"]/i);
  const metaDesc = dm ? dm[1].trim() : 'Comprehensive medical library from Bratton Physical Therapy in Slidell, LA.';

  // library_nl_all is a catch-all parent page - it has the body diagram but no specific article list
  const bodyContent = `<h1>Medical Library</h1>
<p style="font-size:1.15rem;line-height:1.7;margin-bottom:24px">Browse our collection of educational resources, exercise guides, and treatment information. Click a body area or choose a category below.</p>
...
<div style="margin-top:32px">
  <p style="text-align:center;color:#888">Select a body area above or browse categories to the right to find articles on specific conditions and treatments.</p>
</div>

${makeDisclaimer()}
${makeCallButton()}`;

  const outDir = path.join(LIB, 'library_nl_all');
  if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
  const page = buildPage(title, metaDesc, bodyContent);
  fs.writeFileSync(path.join(outDir, 'index.html'), page, 'utf8');
  console.log('Generated: library/library_nl_all/index.html');
}
...
try {
  ...
  console.error('Error generating library_health:', e.message);
}

try {
  generateNlAll();
} catch (e) {
  console.error('Error generating library_nl_all:', e.message);
}
